...
// ============================================================
const LEVEL_CONFIGS = [
  { level: 1, speed: 200, spawnIntervalMs: 1700, gapPx: 195, bgPalette: "dawn",    hasMoving: false, hasWind: false, hasShurikens: false, obstacleTarget: 10 },
  { level: 2, speed: 225, spawnIntervalMs: 1450, gapPx: 178, bgPalette: "day",     hasMoving: false, hasWind: false, hasShurikens: false, obstacleTarget: 12 },
  { level: 3, speed: 245, spawnIntervalMs: 1250, gapPx: 168, bgPalette: "dusk",    hasMoving: true,  hasWind: true,  hasShurikens: false, obstacleTarget: 13 },
  { level: 4, speed: 255, spawnIntervalMs: 1150, gapPx: 160, bgPalette: "night",   hasMoving: false, hasWind: false, hasShurikens: true,  obstacleTarget: 13 },
  { level: 5, speed: 265, spawnIntervalMs: 1050, gapPx: 152, bgPalette: "storm",   hasMoving: true,  hasWind: true,  hasShurikens: true,  obstacleTarget: 14 },
  { level: 6, speed: 280, spawnIntervalMs:  920, gapPx: 145, bgPalette: "inferno", hasMoving: true,  hasWind: true,  hasShurikens: true,  obstacleTarget: 16 },
];
...
class MainScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.scale.on("resize", (gs) => {
      ...
      this.drawBackground();
      this.drawAdPanels();
      if (this.bannerText) this.bannerText.setPosition(this.W / 2, Math.min(90, this.H * 0.2));
      if (this.hintText)   this.hintText.setPosition(this.W / 2, Math.min(125, this.H * 0.27));
    ...
    // ---- UI text ----
    this.scoreText = this.add.text(this.playX + 8, 12, "Score: 0", {
      fontFamily: "system-ui", fontSize: "16px", color: "#ffffff",
      stroke: "#000000", strokeThickness: 4,
    }).setDepth(1000);

    this.comboText = this.add.text(this.playX + 8, 34, "", {
      fontFamily: "system-ui", fontSize: "13px", color: "#ffe066",
      stroke: "#000000", strokeThickness: 4,
    }).setDepth(1000);

    this.progressText = this.add.text(this.playX + 8, 52, `Level 1: 0/10`, {
      fontFamily: "system-ui", fontSize: "14px", color: "#ffffff",
      stroke: "#000000", strokeThickness: 4,
    }).setDepth(1000);

    this.levelBadge = this.add.text(this.W - AD_PANEL_W - 8, 12, "LVL 1", {
      fontFamily: "system-ui", fontSize: "16px", fontStyle: "900",
      color: "#ffe066", stroke: "#000000", strokeThickness: 5,
    }).setOrigin(1, 0).setDepth(1000);

    this.bannerText = this.add.text(this.W / 2, Math.min(90, this.H * 0.2), "", {
    ...
    this.hintText = this.add.text(this.W / 2, Math.min(125, this.H * 0.27), "", {
      fontFamily: "system-ui", fontSize: "16px", fontStyle: "700",
      color: "#ffffff", stroke: "#000000", strokeThickness: 6,
    }).setOrigin(0.5).setVisible(false).setDepth(1000);

    // Wind indicator
    this.windText = this.add.text(this.W / 2, this.H - 28, "", {
      fontFamily: "system-ui", fontSize: "14px", color: "#aaddff",
      stroke: "#000000", strokeThickness: 4,
    }).setOrigin(0.5).setDepth(1000);

    // Deflect prompt
    this.deflectPrompt = this.add.text(this.W / 2, this.H * 0.38, "", {
      fontFamily: "system-ui", fontSize: "22px", fontStyle: "900",
      color: "#ff4444", stroke: "#000000", strokeThickness: 7,
    }).setOrigin(0.5).setVisible(false).setDepth(1010);
    ...
    this.playerBody = this.physics.add.image(spawnX, this.H * 0.5, "__playerBody");
    ...
    this.sitSprite = this.add.image(0, 0, "ninja_sit").setVisible(false).setDepth(60);
  ...
  applyLevelConfig(level) {
    const cfg = LEVEL_CONFIGS.find(c => c.level === level) || LEVEL_CONFIGS[LEVEL_CONFIGS.length - 1];
    this.cfg            = cfg;
  ...
  drawAdPanels() {
    ...
    this.drawAdSlot(this.adGfx, this.W - AD_PANEL_W + 8, 40, AD_PANEL_W - 16, 200, "AD");
    this.drawAdSlot(this.adGfx, this.W - AD_PANEL_W + 8, 260, AD_PANEL_W - 16, 200, "AD");
    ...
    const mk = (x, y, txt) => {
      const t = this.add.text(x, y, txt, {
        fontFamily: "system-ui", fontSize: "10px", color: "#555577",
  ...
  drawAdSlot(g, x, y, w, h, label) {
    ...
    const t = this.add.text(x + w / 2, y + h / 2, label, {
      fontFamily: "system-ui", fontSize: "11px", color: "#334455",
  ...
  drawBackground() {
    ...
    this.bgGfx.fillStyle(pal.mid,    1); this.bgGfx.fillRect(0, 0, this.W, this.H * 0.62);
    this.bgGfx.fillStyle(pal.sky,    1); this.bgGfx.fillRect(0, this.H * 0.62, this.W, this.H * 0.22);
    this.bgGfx.fillStyle(pal.fog, 0.45); this.bgGfx.fillRect(0, this.H * 0.44, this.W, this.H * 0.16);
    this.bgGfx.fillStyle(0x2e8b57, 0.75); this.bgGfx.fillRect(0, this.H - 18, this.W, 18);
  }
  ...
  refreshScenery() {
    if (this.sceneryGroup) this.sceneryGroup.getChildren().forEach(c => c.destroy());
    ...
    g.fillStyle(Phaser.Display.Color.GetColor(
      Math.max(0, mtnCol.r - 30), Math.max(0, mtnCol.g - 30), Math.max(0, mtnCol.b - 30)
    ), 0.5);
    const mtnPts = [];
    for (let i = 0; i <= 12; i++) {
      const x = (this.W / 12) * i;
    ...
    g.fillPoints(mtnPts.reduce((a, v, i) => { if (i % 2 === 0) a.push({ x: v, y: mtnPts[i+1] }); return a; }, []), true);
  ...
  drawPagoda(g, cx, baseY, pal) {
    ...
    // Tiers (bottom to top)
    const tiers = [
      { w: 80, h: 18 }, { w: 64, h: 18 }, { w: 48, h: 16 }, { w: 34, h: 14 },
    ];
    let y = baseY;
    for (const t of tiers) {
      g.fillRect(cx - t.w / 2, y - t.h, t.w, t.h);
      // eave flick
      g.fillTriangle(cx - t.w / 2 - 8, y - t.h, cx - t.w / 2, y - t.h, cx - t.w / 2 + 4, y - t.h - 6);
      g.fillTriangle(cx + t.w / 2 + 8, y - t.h, cx + t.w / 2, y - t.h, cx + t.w / 2 - 4, y - t.h - 6);
      y -= t.h + 2;
    }
    // spire
  ...
  ensureBambooTexture() {
    ...
    const w = 32, h = 256, g = this.add.graphics();
    g.fillStyle(0x4fbf6b, 1); g.fillRect(0, 0, w, h);
    g.fillStyle(0x3aa857, 0.6); g.fillRect(0, 0, 6, h);
    g.fillStyle(0x68d484, 0.35); g.fillRect(w - 7, 0, 7, h);
    for (let y = 20; y < h; y += 24) {
      g.fillStyle(0x2f7f43, 0.95); g.fillRect(0, y, w, 4);
      g.fillStyle(0x2a6f3b, 0.6); g.fillRect(0, y + 4, w, 2);
    }
    g.generateTexture("bamboo", w, h); g.destroy();
  }
  ...
  ensureStarTexture() {
    ...
    g.lineStyle(2, 0xd6b85a, 1); g.strokePath();
    g.generateTexture("star", 32, 32); g.destroy();
  }

  ensureBurstTexture() {
    if (this.textures.exists("burst")) return;
    const w = 256, h = 160, cx = w / 2, cy = h / 2, g = this.add.graphics();
    g.fillStyle(0xffffff, 1); g.lineStyle(6, 0x000000, 1);
    const pts = [];
    for (let i = 0; i <= 18; i++) {
      const a = (Math.PI * 2 * i) / 18, r = i % 2 === 0 ? 72 : 54;
      pts.push({ x: cx + Math.cos(a) * r, y: cy + Math.sin(a) * r });
    }
    g.beginPath(); g.moveTo(pts[0].x, pts[0].y);
    pts.slice(1).forEach(p => g.lineTo(p.x, p.y));
    g.closePath(); g.fillPath(); g.strokePath();
    g.beginPath(); g.moveTo(cx-40,cy+55); g.lineTo(cx-70,cy+80); g.lineTo(cx-25,cy+72); g.closePath(); g.fillPath(); g.strokePath();
    g.generateTexture("burst", w, h); g.destroy();
  }

  ensureSitTexture() {
    if (this.textures.exists("ninja_sit")) return;
    const g = this.add.graphics();
    g.fillStyle(0x1a1a22, 1); g.fillCircle(34, 22, 20);
    g.fillStyle(0x0d0d12, 0.85); g.fillCircle(28, 16, 14);
    g.fillStyle(0xf2d6c9, 1); g.fillEllipse(40, 24, 22, 18);
    g.fillStyle(0x0b0b10, 1); g.fillRoundedRect(39, 22, 9, 3, 1.5);
    g.fillStyle(0x12121a, 1); g.fillRoundedRect(30, 30, 26, 10, 5);
    g.fillStyle(0x14141c, 1); g.fillRoundedRect(28, 40, 18, 16, 7);
    g.fillStyle(0x0f0f16, 1); g.fillRoundedRect(24, 54, 18, 10, 6);
    g.fillStyle(0x14141c, 1); g.fillRoundedRect(38, 54, 18, 10, 6);
    g.fillStyle(0x0a0a10, 1); g.fillRoundedRect(22, 62, 16, 5, 2); g.fillRoundedRect(40, 62, 16, 5, 2);
    g.generateTexture("ninja_sit", 72, 72); g.destroy();
  }
  ...
  ensureShurikenTexture() {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      g.fillTriangle(
        ...
        16 + Math.cos(a+0.45) * 6, 16 + Math.sin(a+0.45) * 6,
        16 + Math.cos(a-0.45) * 6, 16 + Math.sin(a-0.45) * 6
      );
    }
    g.fillStyle(0x888888, 1); g.fillCircle(16, 16, 4);
    g.generateTexture("shuriken", 32, 32); g.destroy();
  }
  ...
  ensureEnemyNinjaTexture() {
    ...
    const g = this.add.graphics();
    // Red-masked enemy
    g.fillStyle(0x8b0000, 1); g.fillCircle(16, 14, 12);
    g.fillStyle(0x500000, 1); g.fillCircle(12, 10, 8);
    g.fillStyle(0xf2d6c9, 1); g.fillEllipse(19, 15, 14, 11);
    g.fillStyle(0xcc0000, 1); g.fillRoundedRect(13, 19, 16, 6, 3);
    g.fillStyle(0x3b0000, 1); g.fillRoundedRect(14, 28, 12, 10, 4);
    g.fillStyle(0x2a0000, 1);
    g.fillRoundedRect(10, 36, 10, 8, 3);
    g.fillRoundedRect(22, 36, 10, 8, 3);
    g.generateTexture("enemy_ninja", 32, 48); g.destroy();
  }
  ...
  buildNinjaRig() {
    ...
    const rr = (w, h, r, col, a=1) => { const g = this.add.graphics(); g.fillStyle(col, a); g.fillRoundedRect(-w/2,-h/2,w,h,r); return g; };

    const head = this.add.container(0, -26);
    const hoodOuter = this.add.graphics(); hoodOuter.fillStyle(0x1a1a22,1); hoodOuter.fillCircle(0,0,20);
    const hoodShadow = this.add.graphics(); hoodShadow.fillStyle(0x0d0d12,0.85); hoodShadow.fillCircle(-6,-6,14);
    const faceOpen = this.add.graphics(); faceOpen.fillStyle(0xf2d6c9,1); faceOpen.fillEllipse(6,4,22,18);
    const eye = this.add.graphics(); eye.fillStyle(0x0b0b10,1); eye.fillCircle(11,2,3.6); eye.fillStyle(0xffffff,1); eye.fillCircle(10,1,1.2);
    const mask = rr(26, 10, 5, 0x12121a); mask.x=6; mask.y=14;
    head.add([hoodOuter, hoodShadow, faceOpen, eye, mask]);

    const torso = rr(18, 18, 7, 0x14141c); torso.y = -2;
    const belt  = rr(20, 4, 2, 0x2c2c38); belt.y = 6;

    const makeLimb = (uLen, lLen, thick, cU, cL) => {
      const limb = this.add.container(0, 0);
      const upper = rr(thick, uLen, Math.ceil(thick/2), cU); upper.y = uLen/2;
      const joint = this.add.container(0, uLen);
      const lower = rr(thick, lLen, Math.ceil(thick/2), cL); lower.y = lLen/2;
      joint.add(lower); limb.add([upper, joint]); limb.joint = joint; limb.lower = lower; return limb;
    };

    const aB = makeLimb(12,10,6,0x0f0f16,0x0f0f16); aB.x=-10; aB.y=-8;
    const aF = makeLimb(12,10,6,0x14141c,0x14141c); aF.x=12;  aF.y=-8;
    const lB = makeLimb(12,12,7,0x0f0f16,0x0f0f16); lB.x=-4;  lB.y=10;
    const lF = makeLimb(12,12,7,0x14141c,0x14141c); lF.x=6;   lF.y=10;

    const foot = col => rr(12,5,2,col);
    const fB = foot(0x0a0a10); fB.y=12; lB.joint.add(fB);
    const fF = foot(0x0a0a10); fF.y=12; lF.joint.add(fF);
  ...
  animateRun(deltaMs) {
    ...
    const p = this.playerRig.parts, t = this.runT * 13.5, s = Math.sin(t);
    const bob = Math.sin(t * 2) * 3.5;

    this.playerRig.x = this.playerBody.x;
    this.playerRig.y = this.playerBody.y + bob;
    this.playerRig.rotation = Phaser.Math.Clamp(this.playerBody.body.velocity.y / 1600, -0.14, 0.14);

    const stretch = 1 + 0.06 * Math.sin(t * 2);
    this.playerRig.scaleY = stretch; this.playerRig.scaleX = 2 - stretch;

    p.legFront.rotation  =  0.85 * s;       p.legBack.rotation  = -0.85 * s;
    p.legFront.joint.rotation = 1.05 + 0.85 * Math.max(0, s);
    p.legBack.joint.rotation  = 1.05 + 0.85 * Math.max(0, -s);
    p.armFront.rotation  = -0.8  * s;       p.armBack.rotation  =  0.8  * s;
  ...
  drawSword() {
    ...
    const bx = x + Math.cos(angle) * 6,  by = y + Math.sin(angle) * 6;
    const tx = x + Math.cos(angle) * 26, ty = y + Math.sin(angle) * 26;
  ...
  handleJump() {
    ...
    if (this.isGameOver || this.isBetweenLevels || this.isVictoryDance || this.isGameComplete) return;
    ...
    if (this.jumpSfx) { this.jumpSfx.stop(); this.jumpSfx.play(); }
  ...
  handleAction() { this.handleJump(); }

  jump() { this.handleJump(); }

  handleEnterAction() {
    if (this.isLeaderboardOpen) {
      if (this.isNameEntry) { this.submitHighScoreIfPending(); return; }
      this.closeLeaderboard(); return;
    }
    if (this.inputLocked) return;
    if (this.isGameOver && this.restartArmed) { this.scene.restart(); return; }
    if (this.isGameComplete) { this.scene.restart(); return; }
    if (this.isVictoryDance) { this.finishVictoryDanceAndAdvance(); }
  }
  ...
  spawnBambooPair() {
    ...
    if (this.cfg.hasShurikens && Math.random() < 0.55) {
      ...
      const en    = this.add.image(side.x, side.y + (side === top ? topH * 0.4 : -botH * 0.4), "enemy_ninja");
      en.setDepth(12);
      en.setScale(0.85);
      en.parentBeam = side;
      en.hasFired   = false;
  ...
  updateShurikens(deltaMs) {
    ...
    this.shurikens = this.shurikens.filter(s => {
      ...
      if (s.x < this.playX - 60) { s.destroy(); return false; }

      // Open deflect window when shuriken enters danger range
      if (!this.deflectWindow && !s.windowOpened &&
          s.x - this.playerBody.x < 180 && s.x > this.playerBody.x - 30) {
        s.windowOpened = true;
  ...
  attemptDeflect() {
    ...
    for (const s of this.shurikens) {
      if (s.windowOpened) { target = s; break; }
    }
    ...
    if (target) {
      ...
      this.comboText.setText(this.combo > 1 ? `⚔ COMBO x${this.combo}! +${bonus}` : "⚔ DEFLECT! +1");
      this.time.delayedCall(900, () => { if (this.comboText) this.comboText.setText(""); });
  ...
  updateWind(deltaMs) {
    ...
    if (this.windTimer >= this.windInterval) {
      ...
      if (this.windText) {
        ...
        this.tweens.add({
          targets: this.windText, alpha: 0.3,
    ...
    if (this.playerBody.x < minX) {
      ...
      if (this.playerBody.body.velocity.x < 0) this.playerBody.body.setVelocityX(0);
    }
  ...
  completeLevel() {
    ...
    const hints = [
      "", "Speed up!", "Gaps tighten…", "Bamboo moves now!", "Ninjas attack — equip sword!", "Full gauntlet. Good luck.",
    ];
    this.hintText.setText(next <= 6 ? `Level ${next}: ${hints[next] || ""}` : "ALL LEVELS CLEARED!");
  ...
  startVictoryDance() {
    if (this.victoryFx) { this.victoryFx.getChildren().forEach(c => c.destroy()); this.victoryFx.destroy(true); this.victoryFx = null; }
    this.victoryFx = this.add.group();
    const cx = this.playerBody.x, cy = this.playerBody.y - 32;
    for (let i = 0; i < 8; i++) {
      const s = this.add.image(cx, cy, "star");
      s.setScale(0.38); s.setAlpha(0.9); s.setDepth(905);
      s.orbitAngle = (Math.PI * 2 * i) / 8; s.orbitRadius = 18 + (i % 2) * 6; s.orbitSpeed = 8;
      this.victoryFx.add(s);
    }
    this.tweens.add({ targets: this.playerRig, duration: 140, y: this.playerRig.y - 18, yoyo: true, repeat: 6, ease: "Sine.easeInOut" });
    this.tweens.add({ targets: this.playerRig, duration: 800, rotation: "+=0.7", ease: "Sine.easeInOut" });
  }
  ...
  finishVictoryDanceAndAdvance() {
    ...
    this.level += 1;
    this.applyLevelConfig(this.level);
    this.obstaclesSpawned = 0; this.obstaclesCleared = 0; this.spawnTimer = 0;
    this.windForce = 0; this.windCurrent = 0; this.windTimer = 0;
    if (this.playerBody?.body) this.playerBody.body.setAccelerationX(0);
    this.shurikenSpawnTimer = 0;
    this.combo = 0;
    ...
    // Sword visibility
    if (!this.hasSword) { this.swordSprite.clear(); this.swordSprite.setVisible(false); }
  ...
  update(_, deltaMs) {
    ...
    if (this.isVictoryDance) {
      ...
      if (this.victoryFx) {
        const dt = deltaMs / 1000, cx = this.playerBody.x, cy = this.playerBody.y - 32;
    ...
    for (const o of this.obstacles) {
      ...
      if (o.moveAmp !== undefined) {
        o.y = o.moveOriginY + Math.sin(this.runT * o.moveSpeed) * o.moveAmp * o.moveDir;
      }
      ...
      if (o.parentBeam !== undefined) {
        ...
        o.x = b.x;
        o.y = b.y + (b.isGate ? b.displayHeight * 0.35 : -b.displayHeight * 0.35);

        // Fire shuriken when close enough
        if (!o.hasFired && o.x < this.playerBody.x + 280 && o.x > this.playerBody.x - 20 && this.deflectCooldown <= 0) {
          o.hasFired = true;
      ...
      if (o.isGate && !o.scored && o.x < this.playerBody.x) {
        ...
        this.scoreText.setText(`Score: ${this.score}`);
        this.progressText.setText(`Level ${this.level}: ${this.obstaclesCleared}/${this.levelTarget}`);
        if (this.obstaclesCleared >= this.levelTarget) { this.completeLevel(); break; }
      }
    }
    ...
    this.obstacles = this.obstacles.filter(o => {
      if (o.x < this.playX - 220) { o.destroy(); return false; }
      return true;
    });
    ...
    for (const o of this.obstacles) {
      ...
      if (o.body && this.playerBody.body && this.physics.overlap(this.playerBody, o)) {
        this.gameOver(); break;
      }
    }
    ...
    if (this.playerBody.x < this.playX + 10) this.playerBody.x = this.playX + 10;
  }
  ...
  gameOver() {
    ...
    if (this.bakaSfx) {
      this.bakaSfx.stop(); this.bakaSfx.play();
      this.bakaSfx.once("complete", () => { if (this.bgm?.isPlaying) this.bgm.stop(); });
    } else {
      if (this.bgm?.isPlaying) this.bgm.stop();
    }

    const burst = this.add.image(this.W / 2, this.H / 2, "burst").setScale(1.05).setDepth(900);
    this.add.text(this.W / 2, this.H / 2 - 6, "BAKA", {
      fontFamily: "system-ui", fontSize: "64px", fontStyle: "900",
      color: "#ff1b1b", stroke: "#000000", strokeThickness: 10,
    ...
    this.time.delayedCall(this.restartDelayMs, () => {
      if (this.isHighScore(this.score)) { this.beginHighScoreEntry(this.score); return; }
  ...
  startDizzyStars() {
    ...
    const cx = this.playerBody.x, cy = this.playerBody.y - 30;
    for (let i = 0; i < 6; i++) {
      const s = this.add.image(cx, cy, "star");
      s.setScale(0.55); s.setAlpha(0.95); s.setDepth(905);
      s.orbitAngle = (Math.PI * 2 * i) / 6;
      s.orbitRadius = 22 + (i % 2) * 6;
      s.orbitSpeed  = 2.2 + (i % 3) * 0.4;
      this.dizzyGroup.add(s);
    }
    this.birdText = this.add.text(cx, cy - 10, "≈  ≈", { fontFamily: "system-ui", fontSize: "18px", color: "#000000" })
      .setOrigin(0.5).setAlpha(0.35).setDepth(905);
    this.dizzyTimer = 0;
  }
  ...
  updateDizzy(deltaMs) {
    ...
    const cx = this.playerBody.x, cy = this.playerBody.y - 30, dt = deltaMs / 1000;
  ...
  // ----------------------------------------------------------
  safeJsonParse(str, fb) { return safeJsonParse(str, fb); }

  loadLeaderboard() {
    return loadLeaderboard(window.localStorage, LEADERBOARD_KEY, LEADERBOARD_MAX);
  }
  ...
  sanitiseName(n) { return sanitiseName(n, 12); }

  isHighScore(score) {
    return isHighScore(window.localStorage, LEADERBOARD_KEY, score, LEADERBOARD_MAX);
  }
  ...
  buildLeaderboardOverlay() {
    ...
    const ts = (size, bold=false) => ({
      fontFamily: "system-ui", fontSize: size, fontStyle: bold ? "900" : "400",
      color: "#ffffff", stroke: "#000000", strokeThickness: bold ? 8 : 5,
    });
    const mono = (size) => ({
      fontFamily: "ui-monospace, Menlo, Monaco, Consolas, monospace", fontSize: size,
      color: "#ffffff", stroke: "#000000", strokeThickness: 5, lineSpacing: 6,
    });

    this.lbTitle  = this.add.text(0,0,"LEADERBOARD", ts("28px",true)).setOrigin(0.5);
    this.lbBody   = this.add.text(0,0,"", mono("15px")).setOrigin(0.5, 0);
    this.lbHint   = this.add.text(0,0,"ESC / ENTER to close • L to toggle", ts("13px")).setOrigin(0.5);

    this.lbNameTitle  = this.add.text(0,0,"NEW HIGH SCORE!", { ...ts("22px",true), color:"#ffe66d" }).setOrigin(0.5).setVisible(false);
    this.lbNamePrompt = this.add.text(0,0,"Name (A–Z / 0–9):", ts("16px",true)).setOrigin(0.5).setVisible(false);
    this.lbNameValue  = this.add.text(0,0,"__________", { ...mono("22px"), fontStyle:"900" }).setOrigin(0.5).setVisible(false);
    this.lbNameHint   = this.add.text(0,0,"Type • ENTER to save • BACKSPACE to delete", ts("13px")).setOrigin(0.5).setVisible(false);

    this.lbPanel.add([this.lbScrim, this.lbBox, this.lbTitle, this.lbBody, this.lbHint, this.lbNameTitle, this.lbNamePrompt, this.lbNameValue, this.lbNameHint]);
    this.layoutLeaderboardOverlay();
  }

  layoutLeaderboardOverlay() {
    if (!this.lbScrim) return;
    this.lbScrim.clear(); this.lbScrim.fillStyle(0x000000,0.55); this.lbScrim.fillRect(0,0,this.W,this.H);
    const bW = Math.min(520, this.W*0.9), bH = Math.min(520, this.H*0.86);
    const bX = (this.W-bW)/2, bY = (this.H-bH)/2;
    this.lbBox.clear();
    this.lbBox.fillStyle(0x111827, 0.92); this.lbBox.fillRoundedRect(bX,bY,bW,bH,18);
    this.lbBox.lineStyle(6,0x000000,0.55); this.lbBox.strokeRoundedRect(bX,bY,bW,bH,18);
    const cx = this.W/2;
    this.lbTitle.setPosition(cx, bY+46);
    this.lbBody.setPosition(cx, bY+86);
    this.lbHint.setPosition(cx, bY+bH-26);
    this.lbNameTitle.setPosition(cx, bY+86); this.lbNamePrompt.setPosition(cx, bY+128);
    this.lbNameValue.setPosition(cx, bY+164); this.lbNameHint.setPosition(cx, bY+200);
  }
  ...
  openLeaderboard({ showNameEntry = false } = {}) {
    if (!this.isLeaderboardOpen) {
      const playing = !this.isGameOver && !this.isVictoryDance && !this.isGameComplete && !this.isBetweenLevels;
      this.wasPlayingWhenLeaderboardOpened = playing;
      if (playing) { this.physics.world.pause(); this.playerBody.setVelocity(0,0); }
  ...
  closeLeaderboard() {
    ...
    this.isLeaderboardOpen = false;
    if (this.wasPlayingWhenLeaderboardOpened) { this.physics.world.resume(); this.inputLocked = false; }
    else if (this.isGameOver && !this.restartArmed) this.inputLocked = true;
    else this.inputLocked = false;
  }

  toggleLeaderboard() {
    if (this.isLeaderboardOpen) { if (!this.isNameEntry) this.closeLeaderboard(); return; }
    if (this.isVictoryDance) return;
    this.openLeaderboard({ showNameEntry: false });
  }

  setNameEntryUiVisible(v) {
    this.lbTitle.setVisible(!v); this.lbBody.setVisible(!v); this.lbHint.setVisible(!v);
    this.lbNameTitle.setVisible(v); this.lbNamePrompt.setVisible(v);
    this.lbNameValue.setVisible(v); this.lbNameHint.setVisible(v);
  }
  ...
  updateNameEntryDisplay() {
    this.lbNameValue.setText((this.nameBuffer||"").padEnd(10,"_").slice(0,10));
  }

  flashNameHint(msg) {
    this.lbNameHint.setText(msg);
    if (this.nameEntryFlashTween) { this.nameEntryFlashTween.stop(); this.nameEntryFlashTween = null; }
    this.lbNameHint.setAlpha(1);
    this.nameEntryFlashTween = this.tweens.add({
      targets: this.lbNameHint, duration: 90, alpha: 0.35, yoyo: true, repeat: 3, ease: "Sine.easeInOut",
      onComplete: () => { if (this.isNameEntry) this.lbNameHint.setText("Type • ENTER to save • BACKSPACE to delete"); this.lbNameHint.setAlpha(1); this.nameEntryFlashTween = null; },
    });
  }
  ...
  handleBackspace(ev) {
    ...
    ev?.preventDefault?.();
    if (this.nameBuffer.length > 0) { this.nameBuffer = this.nameBuffer.slice(0,-1); this.updateNameEntryDisplay(); }
  }

  handleNameTyping(ev) {
    if (!this.isNameEntry || !this.isLeaderboardOpen || !ev || typeof ev.key !== "string") return;
    const k = ev.key;
  ...
  submitHighScoreIfPending() {
    ...
    if (!clean || clean.length < 1) { this.flashNameHint("Please enter a name (at least 1 character)."); return false; }
    this.addHighScore(clean, this.pendingScoreToSave ?? 0);
    const board = this.loadLeaderboard();
    this.lbBody.setText(this.formatLeaderboardText(board));
    this.isNameEntry = false; this.pendingScoreToSave = null; this.nameBuffer = "";
    this.setNameEntryUiVisible(false);
    this.inputLocked = false;
    if (this.isGameOver) { this.restartArmed = true; this.hintText.setText("ENTER to restart • L for leaderboard"); this.hintText.setVisible(true); }
    return true;
  }
}
...
// ============================================================
function start() {
  if (__game) { try { __game.destroy(true); } catch (_) {} __game = null; }
